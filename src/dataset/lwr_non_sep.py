...
class LwrNonSepLoader():
    ...
    def load_data(self):
        ...
        rho_x_t = np.loadtxt(os.path.join("data", "lwr_non_sep", "data_rho_non_sep.csv"), delimiter=',')
        V_x_t = np.loadtxt(os.path.join("data", "lwr_non_sep", "data_V_non_sep.csv"), delimiter=',')
        u_x_t = np.loadtxt(os.path.join("data", "lwr_non_sep", "data_u_non_sep.csv"), delimiter=',')
        ...
        self.train_data["rho"] = rho[:, 0]
        self.train_data["std_rho"] = rho.std()
        self.train_data["mean_rho"] = rho.mean()
        self.train_data["drho_dt"] = (rho - np.append(rho[:,1:],rho[:,-1],1)) / (1/32)

        # terminal cost
        self.train_data["x_of_V"] = x_of_V
        self.train_data["t_of_V"] = t_of_V_obs
        self.train_data["V"] = V[:, -1]
        self.train_data["std_V"] = V.std()
        self.train_data["mean_V"] = V.mean()
...
class LwrNonSepLoaderFullObs():
    ...
    def load_data(self):
        ...
        t_of_rho_obs_idx = np.array([1, 10, 20, -1])
        x_of_V_obs_idx = np.arange(len(x_of_V))
        t_of_V_obs_idx = np.array([1, 10, 20, -1])
        ## ==================================
